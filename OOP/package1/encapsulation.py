...
class TwitterConsoleLogin:
    def __init__(self, consumer_key, consumer_secret):
        self.__consumer = oauth2.Consumer(
    ...
    def __get_request_token(self):
        ...
        if response.status != 200:
            print(
                'An error occured getting the request token from twitter!')
    ...
    def __get_access_token(self, request_token, oauth_verifier):
        token = oauth2.Token(
            ...
            request_token['oauth-token-secret'])
...
twitter_login = TwitterConsoleLogin('mey_key', 'my_secret')
